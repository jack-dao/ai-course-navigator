    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative bg-white w-full max-w-4xl max-h-[90vh] rounded-[32px] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10">
          <div>
            <h2 className="text-3xl font-black text-ucsc-blue tracking-tight">{professor.name.replace(/,/g, ', ')}</h2>
          ...
          <div className="flex items-center gap-3">
            {professor.rmpLink && (
              <a
                href={professor.rmpLink}
                target="_blank"
                rel="noopener noreferrer"
                // Changed to Blue-50 background + Blue-700 text so it pops
                className="px-4 py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-xl transition-all flex items-center gap-2 group font-bold"
                title="View full profile on RateMyProfessors"
              >
                <span className="text-xs hidden sm:block">View on RMP</span>
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-white">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
            {[
              ...
              {
                label: 'Retake',
                val: `${professor.wouldTakeAgain || '0'}%`,
              ...
              <div
                key={i}
                className={`${stat.bg} p-5 rounded-[24px] border border-transparent hover:border-black/5 transition-all flex flex-col items-center text-center gap-1`}
              >
                <div className="p-2.5 bg-white rounded-2xl shadow-sm mb-1">{stat.icon}</div>
          ...
          <div className="space-y-6">
            ...
            {professor.reviews?.length > 0 ? (
              professor.reviews.map((rev, i) => (
                <div
                  key={i}
                  className="bg-slate-50 p-8 rounded-[28px] border border-slate-100 hover:bg-white hover:shadow-xl hover:border-slate-200 transition-all group"
                >
                  ...
                  <p className="text-slate-700 font-medium leading-relaxed text-sm mb-6 pl-1">"{rev.comment}"</p>
                  ...
                  {rev.tags && rev.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-200/50">
                      {rev.tags.map((tag, idx) => (
                        <span
                          key={idx}
                          className="px-2.5 py-1 bg-white border border-slate-200 text-slate-500 text-[10px] font-bold uppercase rounded-md shadow-sm group-hover:border-ucsc-blue/20 transition-colors"
                        >
                          {tag}
                        </span>
                      ))}
