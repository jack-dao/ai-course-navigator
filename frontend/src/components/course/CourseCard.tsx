...
interface CourseCardProps {
  ...
  onShowProfessor: (name: string, ratingData?: ProfessorRating) => void;
  sortOption?: string;
  filters?: CourseFilters;
}

const CourseCard = ({ course, onAdd, professorRatings, onShowProfessor, sortOption, filters }: CourseCardProps) => {
  const [selectedSubSections, setSelectedSubSections] = useState<Record<number, number>>({});
  const [errors, setErrors] = useState<Record<number, boolean>>({});
  const [openDropdownId, setOpenDropdownId] = useState<number | null>(null);
  const dropdownRef = useRef<HTMLDivElement>(null);

  const [descriptionText, setDescriptionText] = useState<string | null>(course.description || null);
  const [prereqText, setPrereqText] = useState<string | null>(course.prerequisites || null);
      ...
      {[...Array(5)].map((_, i) => (
        <div key={i} className="relative">
          ...
          <div
            ...
            style={{ width: `${Math.min(Math.max((rating - i) * 100, 0), 100)}%` }}
          >
            <Star className="w-3 h-3 fill-current text-ucsc-gold" />
          </div>
        </div>
      ))}
    </div>
  ...
  const toggleDropdown = (sectionId: number) => setOpenDropdownId(openDropdownId === sectionId ? null : sectionId);

  const handleSelectDiscussion = (sectionId: number, subId: number | null) => {
  ...
  const formatMetaValue = (type: string, value: string | null): string => {
    if (!value) return '---';
      ...
      <div className="px-6 py-5 bg-white rounded-t-[20px] border-t border-l border-r border-slate-200 border-b border-b-slate-100">
        <div className="flex justify-between items-start mb-3 flex-wrap gap-4">
          <div className="flex-1 min-w-[200px]">
            <div className="flex items-center gap-3 mb-1 flex-wrap">
              <h3 className="text-2xl font-[800] text-ucsc-blue tracking-tight whitespace-nowrap">{course.code}</h3>
          ...
          {geCode && (
            <div className="flex flex-col sm:items-end justify-center sm:ml-auto shrink-0 mt-2 sm:mt-0 max-w-full">
              ...
              <div className="px-3 py-1.5 rounded-xl bg-ucsc-blue/5 border border-ucsc-blue/10 flex items-center gap-2 max-w-full">
                <span className="text-sm font-bold text-ucsc-blue break-words whitespace-normal">
                  {getGEMapping(geCode)} ({geCode})
                </span>
              </div>
            </div>
          )}
        ...
        <div className="mt-4">
          <button
            ...
            className="group flex items-center gap-2 text-sm font-bold text-ucsc-blue hover:text-blue-600 transition-colors outline-none select-none cursor-pointer w-full text-left"
          >
            <div
              className={`p-1 rounded-full bg-slate-100 group-hover:bg-blue-50 transition-colors duration-200 ${showDetails ? 'rotate-180' : ''}`}
            >
          ...
          <div
            className={`grid transition-all duration-300 ease-in-out ${showDetails ? 'grid-rows-[1fr] opacity-100 mt-3' : 'grid-rows-[0fr] opacity-0 mt-0'}`}
          >
                ...
                {isLoadingDetails ? (
                  ...
                  <>
                    <div className="flex gap-3 items-start">
                      <div className="p-1.5 bg-white rounded-lg border border-slate-100 shrink-0 text-ucsc-blue">
                        <BookOpen className="w-4 h-4" />
                      </div>
                      <div className="space-y-1">
                        <span className="text-sm font-bold text-ucsc-blue mb-0.5 block">Description</span>
                    ...
                    {prereqText && (
                      <div className="flex gap-3 items-start pt-4 border-t border-slate-200/60">
                        ...
                        <div className="space-y-1">
                          <span className="text-sm font-bold text-ucsc-blue mb-0.5 block">Prerequisites</span>
                          <div className="text-slate-700 font-medium leading-relaxed break-words whitespace-normal">
        ...
        {sortedSections.length > 0 ? (
          sortedSections.map((section, index) => {
            ...
            return (
              <div
                key={section.id}
                className={`p-6 hover:bg-slate-50/50 transition-colors ${isLast ? 'rounded-b-[20px]' : ''}`}
              >
                <div className="flex flex-wrap gap-6">
                  <div className="flex-[1_1_100%] md:flex-[1_0_300px] flex flex-col gap-4 min-w-0 md:min-w-[300px]">
                    <div className="w-full shrink-0">
                      <p className="text-[10px] font-bold text-ucsc-blue mb-1 uppercase tracking-wider">Instructor</p>
                      <button
                        onClick={() => onShowProfessor(section.instructor, ratingData)}
                        className="flex items-start gap-2 group/prof text-left cursor-pointer w-full min-w-0"
                      >
                        <div className="w-10 h-10 rounded-full bg-ucsc-blue/5 flex items-center justify-center text-ucsc-blue shrink-0">
                          <User className="w-5 h-5" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <span className="block font-bold text-slate-900 group-hover/prof:text-ucsc-blue group-hover/prof:underline decoration-2 underline-offset-2 transition-colors leading-tight truncate">
                    ...
                    <div className="grid grid-cols-2 gap-y-3 gap-x-3 sm:gap-x-6">
                      <div className="min-w-0">
                        <p className="text-[10px] font-bold text-ucsc-blue mb-0.5 whitespace-nowrap">Class Number</p>
                        <div className="flex items-center gap-1.5 text-xs font-black text-slate-900">
                          <Hash className="w-3.5 h-3.5 text-ucsc-blue shrink-0" />
                          <span className="truncate">{section.classNumber || '---'}</span>
                        </div>
                      </div>
                      <div className="min-w-0">
                        <p className="text-[10px] font-bold text-ucsc-blue mb-0.5 whitespace-nowrap">Instruction</p>
                        <div className="flex items-center gap-1.5 text-xs font-black text-slate-900">
                          <Monitor className="w-3.5 h-3.5 text-ucsc-blue shrink-0" />
                          <span className="truncate">{section.instructionMode || 'In Person'}</span>
                        </div>
                      </div>
                      <div className="min-w-0">
                        <p className="text-[10px] font-bold text-ucsc-blue mb-0.5 whitespace-nowrap">Career</p>
                        <div className="flex items-center gap-1.5 text-xs font-black text-slate-900">
                          <GraduationCap className="w-3.5 h-3.5 text-ucsc-blue shrink-0" />
                          <span className="truncate">{formatMetaValue('Career', career || 'Undergraduate')}</span>
                        </div>
                      </div>
                      <div className="min-w-0 pr-2">
                        <p className="text-[10px] font-bold text-ucsc-blue mb-0.5 whitespace-nowrap">Grading</p>
                        <div className="flex items-center gap-1.5 text-xs font-black text-slate-900">
                          <BookOpen className="w-3.5 h-3.5 text-ucsc-blue shrink-0" />
                          <span className="whitespace-normal break-words">
                            {formatMetaValue('Grading', grading ? 'Student Option' : 'Letter')}
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex-[1_0_200px] flex flex-col justify-center min-w-[200px] pt-4 border-t border-dashed border-slate-200 md:border-t-0 md:pt-0 md:border-l md:pl-6">
                    ...
                    <div className="space-y-1.5 w-full">
                      <div className="flex justify-between items-end">
                        <span
                          className={`text-[10px] font-bold uppercase tracking-wider ${isClosed ? 'text-rose-600' : isWaitlist ? 'text-orange-600' : 'text-ucsc-blue'}`}
                        >
                          {section.status || 'Open'}
                        </span>
                  ...
                  <div className="flex-[1_0_220px] flex flex-col gap-2 justify-center min-w-[220px] 2xl:border-l 2xl:pl-6 border-slate-200 border-dashed">
                    {hasDiscussions && (
                      <div
                        className={`relative ${openDropdownId === section.id ? 'z-50' : 'z-0'}`}
                        ref={openDropdownId === section.id ? dropdownRef : null}
                      >
                        <button
                        ...
                        >
                          ...
                          <ChevronDown
                            className={`w-3.5 h-3.5 transition-transform ${openDropdownId === section.id ? 'rotate-180' : ''}`}
                          />
                        </button>
                            ...
                            <div className="max-h-96 overflow-y-auto custom-scrollbar p-1">
                              {selectedSubId && (
                                <button
                                  ...
                                  className="w-full text-left px-3 py-2.5 text-xs font-bold text-rose-500 hover:bg-rose-50 rounded-lg mb-1 flex items-center gap-2 cursor-pointer"
                                >
                      ...
                      className={`w-full py-3 rounded-xl font-bold text-xs flex items-center justify-center gap-2 shadow-sm transition-all active:scale-95 cursor-pointer ${
                        isClosed
                          ...
                          : isWaitlist
                            ...
                            : 'bg-ucsc-blue text-white hover:bg-ucsc-blue-dark hover:shadow-md'
                      }`}
                    >
                      {isClosed ? (
                        <>
          ...
          <div className="p-6 text-center text-slate-400 text-xs italic">No sections match your specific filters.</div>
        )}
      </div>
    </div>
  );
