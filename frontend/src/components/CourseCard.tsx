...
const CourseCard = ({ course, onAdd, professorRatings, onShowProfessor, sortOption, filters }: CourseCardProps) => {
  ...
  const formatLocation = (loc: string | undefined): string => loc ? loc.toLowerCase().split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') : 'TBA';

  const expandDays = (daysStr: string | undefined): string => {
    if (!daysStr || daysStr === 'TBA') return 'TBA';
    const map: Record<string, string> = { M: 'Monday', Tu: 'Tuesday', W: 'Wednesday', Th: 'Thursday', F: 'Friday', Sa: 'Saturday', Su: 'Sunday' };
