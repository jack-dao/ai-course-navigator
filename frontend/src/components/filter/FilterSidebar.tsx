...
const FilterSidebar = ({ filters, setFilters, onReset, activeTab, onClose }: FilterSidebarProps) => {
      ...
      <div className="p-6 pt-2">
        <FilterSection title="Department">
          <CustomDropdown
            ...
            onChange={(val: string) => setFilters((prev) => ({ ...prev, department: val }))}
          />
        </FilterSection>
          ...
          <div className="flex justify-between gap-1">
            {['M', 'Tu', 'W', 'Th', 'F'].map((day) => (
              <button
                ...
                className={`w-10 h-10 rounded-xl text-[10px] font-bold transition-all border-2 shadow-sm cursor-pointer ${filters.days.includes(day) ? 'bg-ucsc-blue text-white border-ucsc-blue shadow-md scale-105' : 'bg-white text-slate-600 border-slate-200 hover:border-ucsc-gold hover:text-ucsc-blue'}`}
              >
                {day}
              </button>
            ))}
          ...
          <div className="px-2 py-4">
            <div className="relative h-4 flex items-center">
              <div className="absolute w-full h-1.5 bg-slate-200 rounded-full">
                <div
                  className="absolute h-full bg-ucsc-gold opacity-60 rounded-full left-0"
        ...
        <FilterSection title="Time Range">
          <div className="px-2 py-4">
            <div className="relative h-4 flex items-center">
              <div className="absolute w-full h-1.5 bg-slate-200 rounded-full">
                <div
                  className="absolute h-full bg-ucsc-gold opacity-60 rounded-full"
                  style={{
        ...
        <FilterSection title="Instructor Rating">
          <div className="px-2 py-2">
            <input
              ...
              className="w-full h-1.5 bg-slate-200 rounded-full appearance-none cursor-pointer accent-ucsc-blue"
            />
            <div className="flex justify-between mt-3 text-[10px] font-bold text-slate-500">
              <span className="opacity-50">Any</span>
              <div className="flex items-center gap-1 text-ucsc-blue">
                <span className="text-lg font-black">{filters.minRating}+</span>
                <Star className="w-3 h-3 fill-ucsc-gold text-ucsc-gold" />
              </div>
        ...
        <FilterSection title="Availability">
          <label className="flex items-center gap-3 cursor-pointer group p-3 bg-white border border-slate-200 rounded-xl shadow-sm hover:border-ucsc-gold transition-all">
            <input
              type="checkbox"
              checked={filters.openOnly}
              onChange={() => setFilters((prev) => ({ ...prev, openOnly: !prev.openOnly }))}
              className="accent-ucsc-blue cursor-pointer w-4 h-4"
            />
